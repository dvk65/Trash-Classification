...
trash_classification_using_ResNet.ipynb
  |
  Dataset (unzip dataset before uploading. code does not hande that currently. hpc so do not need to worry about it either mostly)
    |
    cups
    cans
    bottles
...
Resnet50 is better at training higher complexity tasks with limited dataset. Apparently it is good with distingushing backgrounds and objects (deeper networks) and pattern recognition becasue does not have the vanishing gardient problem (where the model forgets information as it proceeds along the network). Hence ResNet good. Gave us validation (or testing) accuracy of 98% with the current trashclassify.keras file in this repo.</br>

### Step 3:
It needed some work to upload the trashclassify.keras file downloaded from hpc to GitHub.
...
'''"2.16.1\n",
      "3.3.3\n"'''
They are compatible and do the zipping things too.
It took a few tries to upload it to GitHub. Also tried converting to tflite but some model layers were causing issues.
Steps to upload to GitHub using lfs:
...
### Step 4:
app.py takes the saved model and uses it on the front-end.
Chose Streamlit because it is meant to be used as front-ends for projects with models in the backend.

## Optional To-dos:
1. Pipeline:
Think of a pipeline to combine the notebook, dataset and GitHub repo code.
Something to make it more easily reproducible.
2. Dataset:
Add more directories with over 1100 images.
See something_to_think_about(1 and 2).png to add more directories.
3. Should we add a part to the front-end to keep updating the dataset? Add targets as options to choose from (like wrappers, cups, bottles, etc). When people click on it, they can click pictures of that object and upload it to that directory.
